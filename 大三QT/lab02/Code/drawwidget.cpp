...
void DrawWidget::paintEvent (QPaintEvent *)
{
    ...
    static double width = rect().width();
    static double height = rect().height();
    painter.setRenderHint(QPainter::Antialiasing, true);//抗锯齿

    double new_width = rect().width()/width;
    double new_height = rect().height()/height;

    double min = qMin(new_width,new_height);
...
void DrawWidget::savePic()
{
    ...
    QString fileName=tr("D:/Project/lab02_%1.png").arg(currentDate);
